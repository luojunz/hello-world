...
myH1.textContent = jsonObj['headline1'];      //将它的 textContent 设为 JSON 对象的 squadName 属性，
...
myPara.textContent =  jsonObj['content1'] ;
header.appendChild(myPara);

var myH1 = document.createElement("h1");      //这儿我们首先使用 createElement() 创建了一个 <h1> 节点，
myH1.textContent = jsonObj['headline2'];      //将它的 textContent 设为 JSON 对象的 squadName 属性，
header.appendChild(myH1);

var myPara = document.createElement('p');     //唯一的不同在于它的内容设为一个与 JSON 内属性 homeTown 和formed相关联的字符串。
myPara.textContent =  jsonObj['content2'] ;
header.appendChild(myPara);
